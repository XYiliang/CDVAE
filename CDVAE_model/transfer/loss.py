...
from CDVAE_model.components.decoder import DiscriminatorDomain, DiscriminatorTC
...
class CDVAELoss:
    def __init__(self, vae, device, dim_dict, cfg):
        ...
        # Optimizer
        self.vae_opt = optim.Adam(self.vae.parameters(), lr=cfg.lr)
    ...
    def calc_src_loss(self, data_s, alpha):
        ...
        del xnc, xnb, xac, xab, a, y, domain_tag, xn, xa, factual, counter,\
    ...
    def calc_trg_loss(self, data_t, alpha):
        ...
        factual, _, domain_pred, cf_dis = self.vae(xn, xa, a, alpha)
        un, ua, mu, logvar, r_xnc, r_xnb, r_xac, r_xab, r_y = factual
        cf_U_dis, cf_ua_dis = cf_dis
        logvar = self.diagonal(logvar)
