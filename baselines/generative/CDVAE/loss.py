...
from CDVAE_model.single_domain.discriminator import DiscriminatorTC
...
class CDVAELoss:
    def __init__(self, vae, device, dim_dict, cfg):
        ...
        self.loss_arg = {k: 1 if v != 0 else 0 for k, v in self.dims.items()}   
    ...
    def optimize(self, data):
        ...
        # VAE Optimize
        # Reconstruction Loss
        xnc_loss = self.loss_fn_cont(r_xnc, xnc) * self.loss_arg['xnc']
        xnb_loss = self.loss_fn_bin(r_xnb, xnb) * self.loss_arg['xnb']
        xac_loss = self.loss_fn_cont(r_xac, xac) * self.loss_arg['xac']
        xab_loss = self.loss_fn_bin(r_xab, xab) * self.loss_arg['xab']
