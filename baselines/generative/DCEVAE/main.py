...
parser.add_argument('--batch_size', type=int, default=1024, help='number of gpu')
...
from utils import setup_logger, make_loader, make_balancing_loader
...
def main(args):
    ...
    train_loader, test_loader, real_cf_test_loader, input_dim = make_loader(args)

    args.input_dim = input_dim
    model = DCEVAE(rc_dim=input_dim['rc'], rb_dim=input_dim['rb'], dc_dim=input_dim['dc'], db_dim=input_dim['db'], sens_dim=input_dim['a'], label_dim=input_dim['y'], args=args).to(args.device)
    ...
    if args.test == True:
        print('Test Start')
        test(test_loader, real_cf_test_loader, args, logger_2)

        '''Generate Data'''
        # print('Generate Data')
        # generate_data(train_loader, args, 'train')
        # generate_data(test_loader, args, 'test')
        # generate_data(real_cf_test_loader, args, 'real_cf')

    '''Fair Classifier'''
    # if args.clf == True:
    #     # baseline_classifier(train_loader, test_loader, args, logger_3)
    #     fair_whole_classifier(train_loader, test_loader, real_cf_test_loader, args, logger_3)
