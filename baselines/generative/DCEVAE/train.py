...
def train(model, train_loader, args, logger):
    device = args.device
    model.to(device)
    ...
    for epoch_i in trange(args.n_epochs):
        ...
        for idx, (rc, rb, dc, db, a, y, rc2, rb2, dc2, db2, a2, y2) in enumerate(train_loader):
            ...
            D_tc_loss_val.backward(retain_graph=True)
        ...
        logger.info('###Train###')
        logger.info('MSE(x): {:.4f}'.format(np.array(train_x_recon_losses[-len(train_loader):]).mean()))
        logger.info('MSE(y): {:.4f}'.format(np.array(train_y_recon_losses[-len(train_loader):]).mean()))
