...
class CEVAE(nn.Module):
    def __init__(self, x_dim, o_dim, sens_dim, label_dim, args):
        ...
        # rc_dim=1, dc_dim=2, db_dim=1(dummy, actually0)
        self.mu_au_to_xc = nn.Sequential(nn.Linear(i_dim, i_dim), act_fn, nn.Linear(i_dim, 3))
        self.mu_au_to_xb = nn.Sequential(nn.Linear(i_dim, i_dim), act_fn, nn.Linear(i_dim, 1))
    ...
    def p_oxy(self, a, u):
        ...
        xc_mu = self.mu_au_to_xc(intermediate)
        xb_mu = self.mu_au_to_xb(intermediate)
        x_mu = torch.cat([xc_mu, xb_mu], 1)
    ...
    def reconstruct_hard(self, a, u):
        ...
        xc, xb = x_mu[:, :-1], x_mu[:, -1].view(-1, 1)
        xc_cf, xb_cf = x_cf_mu[:, :-1], x_cf_mu[:, -1].view(-1, 1)
        ...
        xb = nn.Sigmoid()(xb)
        xb_cf = nn.Sigmoid()(xb_cf)
        xb_hard = dists.bernoulli.Bernoulli(xb)
        xb_hard = xb_hard.sample().type(torch.float64)
        xb_cf_hard = dists.bernoulli.Bernoulli(xb_cf)
        xb_cf_hard = xb_cf_hard.sample().type(torch.float64)
        x_hard = torch.cat([xc, xb_hard], 1)
        x_cf_hard = torch.cat([xc_cf, xb_cf_hard], 1)
    ...
    def calculate_loss(self, x, o, a, y):
        ...
        recon = loss_fn_cont(x_p[:, :-1], x[:, :-1])
        recon += loss_fn_bin(o_p, o)
        recon = recon / MB
