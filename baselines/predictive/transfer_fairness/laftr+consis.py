...
from transfer_fairness.utils import *
from transfer_fairness.dalib.modules.domain_discriminator import DomainDiscriminator
from transfer_fairness.dalib.adaptation.dann import LaftrLoss, LaftrLoss_DP
from transfer_fairness.make_losses import *
import torch.multiprocessing
import torch.optim as optim
from tqdm import tqdm

parser = ArgumentParser()
parser.add_argument('--dataset', choices=['shapes', 'newadult', 'utk-fairface', 'synthetic'],
                    default='newadult')
...
parser.add_argument('--model', choices=['vgg16', 'resnet18', "mlp", 'cnn'], default='mlp')
...
parser.add_argument('--threshold', default=0.95, type=float,
                    help='pseudo label threshold for confidence')
###################################################
parser.add_argument('--fair-consis', action='store_true', default=True,
...
parser.add_argument('--transform_type', type=str, choices=['crop', 'crop_pad'], default='crop',
                    help="Only use for shapes dataset. Sshift1 and Sshift2 use crop. Dshift and Hshift use crop_pad")
...
def train_model(args, model, loaders):
    ...
    t = tqdm(range(args.epoch), leave=True, position=0, unit='epoch')

    for epoch in t:
        start = default_timer()
        # train
        s_train_loss, s_train_prec, s_train_unfair = \
            train_loop(args, epoch, (s_train_dataloader, t_train_dataloader),  model, fair_adv, optimizer)

        # validation
        if epoch % args.val_epoch == args.val_epoch - 1:
            # test in the source domain
            s_val_loss, s_val_prec, s_val_unfair_var, s_val_unfair_odd, s_result = eval_loop_consis(
                args, epoch, 'source', s_test_dataloader, model)
            results.append(s_result)

            # test in the target domain
            t_val_loss, t_val_prec, t_val_unfair_var, t_val_unfair_odd, t_result = eval_loop_consis(
                args, epoch, 'target', t_test_dataloader, model)
        ...
        print('epoch:', default_timer()-start, 's')

    # save results to csv
    fields = ["name", "epoch", "domain", "acc", "acc_A0Y0", "acc_A0Y1", "acc_A1Y0", "acc_A1Y1",
              "consis_A0Y0",
              "consis_A0Y1", "consis_A1Y0", "consis_A1Y1", "acc_var", "acc_dis", "err_op_0",
              "err_op_1",
              "err_odd"]
...
def train_loop(args, epoch, dataloaders, model, fair_adv, optimizer):
    ...
    for batch_idx in range(args.train_iteration):
        print(f'{batch_idx+1}/{args.train_iteration}')
        start = default_timer()
        # load source data
        # try:
        ...
        # forward
        # model接受原样本作为输入，与VAE重构一样。可以直接输入反事实
        y, f = model(inputs)
        f_0 = f[(groups == 0)]
        f_1 = f[(groups == 1)]
        ...
        if args.normalize_loss:
            loss = loss / (
                        1 + args.fair_weight + args.consis_weight_source + args.consis_weight_target)

        # back propagation
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        print('train_loop:', default_timer()-start, 's')
...
if __name__ == "__main__":
    ...
    model = main(args)
    # torch.save(model, f'transfer_fairness/result/transfer_fairness_clf_{args.dataset}.pth')
    torch.save(model, f'transfer_fairness/result/transfer_fairness_clf_new_adult_CA_ALL.pth')
    print('model_saved')
