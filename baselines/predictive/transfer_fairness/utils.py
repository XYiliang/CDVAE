...
from transfer_fairness import data_loader
import pickle
from transfer_fairness.make_models import *
from random import randrange
import numpy as np
from transfer_fairness.randaugment import RandAugment_face as RandAugment
import csv
from transfer_fairness.metrics import *
...
def load_data(args):
    if args.dataset == 'synthetic':
        ...
        s_train_dataset = data_loader.SyntheticDataset(phase='train', domain='source')
        s_test_dataset = data_loader.SyntheticDataset(phase='test', domain='source')
        t_train_dataset = data_loader.SyntheticDataset(phase='train', domain='target')
        t_test_dataset = data_loader.SyntheticDataset(phase='test', domain='target')
        ...
        t_train_dataset = data_loader.NewAdultDataset(args.data_root, args.target_state, True,
                                                      '2018', task=args.state_task,
                                                      phase='train')
        t_test_dataset = data_loader.NewAdultDataset(args.data_root, args.target_state, True,
                                                     '2018', task=args.state_task,
                                                     phase='test')
        print('loading datasets done')
...
def load_model(args):
    if args.dataset == 'newadult':
        ...
        if args.model == 'mlp':
            model = Synthetic_MLP(args.num_labels)
        else:
            raise Exception(f'Only support model type: mlp')
        return model
    # if args.dataset == 'utk-fairface':
...
def fill_args(args):
    if args.dataset == 'newadult':
        args.data_root = 'datasets/ACSIncome/data'
        args.save_path = 'checkpoint/newadult'
        args.save_csv_path = 'transfer_fairness/result'
        args.save_name = 'transfer_fairness_result_new_adult'
        args.val_epoch = 2
        ...
        args.save_path = 'checkpoint/synthetic'
        args.save_csv_path = 'transfer_fairness/result'
        args.save_name = 'transfer_fairness_result_synthetic'
        args.val_epoch = 2
        args.adv_hidden_dim = 128
        args.num_workers = 1
        args.epoch = 10
