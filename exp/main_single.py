...
def main(cfg):
    ...
    vae_savename = f'vae_single(xn{cfg.parm_r_xn}_xa{cfg.parm_r_xa}_y{cfg.parm_r_y}_kl{cfg.parm_kl}_' \
                   f'tc{cfg.parm_tc}_css{cfg.parm_css}_uxn{cfg.uxn_dim}_uxa{cfg.uxa_dim}_seed{cfg.seed}).pth'
    ...
    logger.info(f'Main EXP Config: Dataset:{cfg.dataset_name}|Random Seed:{cfg.seed}|VAE Epochs:{cfg.n_epochs}'
                f'VAE lr:{cfg.lr}| Discriminator lr:{cfg.disc_lr}||Loss Weights: Xn:{cfg.parm_r_xn} '
    ...
    model = train(model, cfg.n_epochs, writer, loaders['train_loader'], logger, device, loaders['dim_dict'], cfg)
