...
def save_predictors(model, vae_savename, cfg):
    if cfg.is_transfer:
    ...
    model.cpu()
    predictor_savename = re.sub(r'vae_.*\(', 'clf(', vae_savename.replace(').pth', '')) +\
...
def ini_result_dataframe(dataset_name, task, is_transfer=False):
    ...
    if task == 'classification':
        metrics = ['Accuracy', 'AUC', '|TE|', 'CFD', 'CLP', 'MMD_rf', 'MMD_rc', 'MMD_cf']
    else:
    ...
    if is_transfer:
        ...
        if dataset_name == 'synthetic':
            for term in ['TE', 'CFD']:
                multi_index = multi_index.insert(loc=len(multi_index.values), item=('real_cf', term))
    multi_index = multi_index.insert(loc=0, item=('MMD', 'mmd_latent_code'))
